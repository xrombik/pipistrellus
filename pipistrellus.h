...
/** применён при определении, что бы не выполнять эту операцию во время выполнения */
...
#define ICMP_ROTO                     1        /**< Значение поля icmp_frame::proto */
...
typedef struct
{
  ...
  uint32_t size_used;   /**< Размер области в байтах занятый, т.е.
                             область из которой можно читать */
  uint8_t* data;        /**< Указатель на область памяти */
} buffer;
...
typedef struct
{
  uint8_t dst[6]; /**< mac-адрес назначения (кому) */
  uint8_t src[6]; /**< mac-адрес источника (от кого) */
...
typedef struct
{
  ...
  uint8_t time[8];        /**< Отметка времени. В ответе повторяет значение из запроса */
...
typedef struct
{
	...
	uint8_t     protocol; /**< ip поле, код протокола */
	uint16_t    xsum;     /**< ip поле, контрольная сумма */
	uint32_t    srcadr;   /**< ip поле, ip-адрес отправителя */
	uint32_t    dstadr;	  /**< ip поле, ip-адрес получателя */
	uint16_t    srcport;  /**< udp поле, ip-порт отправителя */
	uint16_t    dstport;	/**< udp поле, ip-порт получателя */
...
/** Проверяет, что icmp-запрос предназначен для текущего узла
 ...
 \return true если icamp-запрос адресован текущему узлу и false если иначе */
bool icmp_receive(const buffer* rx_buffer, const mac_addrs* maddrs, uint32_t ip_addr);
